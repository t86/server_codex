...
TARGET_ALIASES=(${TARGET_ALIASES:-ecs-111 ecs-114})
...
ssh-keyscan -H 43.155.136.111 43.155.163.114 >> "$REMOTE_KEY_DIR/known_hosts" 2>/dev/null || true
...
ssh "$CONTROL_ALIAS" "ssh -F '$REMOTE_KEY_DIR/config' -o BatchMode=yes ecs-111 'echo ecs-111-ok' && ssh -F '$REMOTE_KEY_DIR/config' -o BatchMode=yes ecs-114 'echo ecs-114-ok'"
